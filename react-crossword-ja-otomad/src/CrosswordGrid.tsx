...
export default function CrosswordGrid({ theme }: CrosswordGridProps) {
  ...
  const [ratio, setRatio] = useState<[number, number]>([100, 100]);
  // useEffect(() => {
  //   if (gridData) {
  ...
  //         const cells = gridData[y];
  //         if (cells.every((c) => !c.used)) {
  //           maxRows = y;
  //           break;
  //         }
  //       }
  ...
  //         const cells = gridData[x];
  //         for (let y = cells.length; maxCols < y; y--) {
  //           if (!cells.at(y)?.used) {
  //             maxCols = y;
  //             break;
  //           }
  ...
  //       if (maxCols > maxRows) {
  //         // console.log(100, (100 / maxCols) * maxRows);
  //         setRatio([100, (100 / maxCols) * maxRows]);
  //       } else {
  //         // console.log((100 / maxRows) * maxCols, 100);
  //         setRatio([(100 / maxRows) * maxCols, 100]);
  //       }
  //     }
  //   }
  // }, [gridData, setRatio]);
        ...
        <GridWrapper>
          ...
          */}
          <div style={{ margin: 0, padding: 0, position: 'relative' }}>
            <svg viewBox={`0 0 ${ratio[0]} ${ratio[1]}`}>
              <rect
                x={0}
                y={0}
                width={ratio[0]}
